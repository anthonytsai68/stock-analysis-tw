...
class Config:
    ...
    gemini_model_fallback: str = "gemini-3-flash-preview"  # 備選模型
    ...
    # Anthropic Claude API（備選，當 Gemini 不可用時使用）
    ...
    # OpenAI 兼容 API（備選，當 Gemini/Anthropic 不可用時使用）
    openai_api_key: Optional[str] = None
    openai_base_url: Optional[str] = None  # 如: https://api.openai.com/v1
    openai_model: str = "gpt-5.5"  # OpenAI 兼容模型名稱
    ...
    # === 新聞與分析篩選配置 ===
    news_max_age_days: int = 3   # 新聞最大時效（天）
    news_strategy_profile: str = "short"  # 新聞窗口策略檔位：ultra_short/short/medium/long
    news_intel_retention_days: int = 30  # 本地資訊池保留天數
    news_intel_fetch_timeout_sec: float = 8.0  # 單個資訊源拉取超時
    news_intel_max_items_per_source: int = 50  # 單次每個資訊源最多采集條數
    news_intel_auto_fetch_enabled: bool = False  # 是否在分析前自動初始化並拉取本地資訊源
    newsnow_base_url: str = "https://newsnow.busiyi.world"  # NewsNow HTTP API base URL (數據源側，不影響 LLM/provider base URL)
    bias_threshold: float = 5.0  # 乖離率閾值（%），超過此值提示不追高
    ...
    feishu_webhook_secret: Optional[str] = None  # 自定義機器人簽名密鑰（可選）
    feishu_webhook_keyword: Optional[str] = None  # 自定義機器人關鍵詞（可選）
    dingtalk_webhook_url: Optional[str] = None
    dingtalk_secret: Optional[str] = None

    # 飛書應用機器人（App Bot）通知
    feishu_chat_id: Optional[str] = None  # 目標群會話 chat_id（群聊模式），或用戶 open_id（P2P 模式）
    feishu_receive_id_type: str = "chat_id"  # 接收者 ID 類型: "chat_id"(群聊) / "open_id"(私聊)
    feishu_domain: str = "feishu"  # 飛書域名: "feishu"(feishu.cn) / "lark"(larksuite.com)
    ...
    email_sender_name: str = "daily_stock_analysis股票分析助手"  # 發件人顯示名稱
    ...
    # Gotify 配置（server base URL；sender 會拼接 /message）
    ...
    custom_webhook_body_template: Optional[str] = None  # 自定義 Webhook JSON body 模板
    webhook_verify_ssl: bool = True  # Webhook HTTPS 證書校驗，false 可支持自簽名（有 MITM 風險）
    ...
    # 通知降噪機制（Issue #1200 P4）：默認全部關閉，僅對靜態通知渠道生效
    ...
    # 僅分析結果摘要：true 時只推送彙總，不含個股詳情（Issue #262）
    ...
    pushplus_topic: Optional[str] = None  # PushPlus 群組編碼（一對多推送）
    ...
    discord_max_words: int = 2000  # Discord 限制 2000 字，默認 2000 字
    wechat_msg_type: str = "markdown"  # 企業微信消息類型，默認 markdown 類型

    # Markdown 轉圖片（Issue #289）：對不支持 Markdown 的渠道以圖片發送
    markdown_to_image_channels: List[str] = field(default_factory=list)  # 逗號分隔：telegram,wechat,custom,email
    markdown_to_image_max_chars: int = 15000  # 超過此長度不轉換，避免超大圖片
    md2img_engine: str = "wkhtmltoimage"  # wkhtmltoimage | markdown-to-file (Issue #455, better emoji support)

    # 實時行情預取（Issue #455）：設為 false 可禁用，避免 efinance/akshare_em 全市場拉取
    ...
    market_review_enabled: bool = True        # 是否啟用大盤復盤
    daily_market_context_enabled: bool = True   # 是否將大盤環境摘要用於個股分析 Prompt 與保守護欄
    # 大盤復盤市場區域：cn(A股)、hk(港股)、us(美股)、jp(日股)、kr(韓股)、both(全部市場)
    market_review_region: str = "cn"
    market_review_color_scheme: str = "green_up"
    # 交易日檢查：默認啟用，非交易日跳過執行；設為 false 或 --force-run 可強制執行（Issue #373）
    trading_day_check_enabled: bool = True

    # === 實時行情增強數據配置 ===
    # 實時行情開關（關閉後使用歷史收盤價進行分析）
    enable_realtime_quote: bool = True
    # 盤中實時技術面：啟用時用實時價計算 MA/多頭排列（Issue #234）；關閉則用昨日收盤
    enable_realtime_technical_indicators: bool = True
    # 籌碼分佈開關（該接口不穩定，雲端部署建議關閉）
    ...
    # - efinance/akshare_em: 東財全量接口，數據最全但容易被封
    ...
    # === 基本面聚合開關與降級保護 ===
    # 全局總開關；關閉時返回 not_supported 並保持主流程無變化
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        加載優先級：
        1. 大多數配置保持系統環境變量優先
        2. WebUI 可寫的運行期關鍵鍵優先複用持久化 `.env`，但保留啟動時顯式進程環境變量的 override
        3. 代碼中的默認值
        """
        ...
        if http_proxy:
            ...
            # 設置環境變量 (requests/urllib3/aiohttp 都會遵守此設置)
            os.environ['NO_PROXY'] = final_no_proxy
            os.environ['no_proxy'] = final_no_proxy

            # 確保 HTTP_PROXY 也被正確設置（以防僅在 .env 中定義但未導出）
        ...
        if invalid_searxng_urls:
            logger.warning(
                "SEARXNG_BASE_URLS 中存在無效 URL，已忽略: %s",
    ...
    def _channels_to_model_list(cls, channels: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        - LiteLLM model_list 語義: https://docs.litellm.ai/docs/proxy/configs#the-model_list-key
        """
    ...
    def _legacy_keys_to_model_list(
        ...
        Compatibility note:
        - LiteLLM OpenAI-compatible 約定: https://docs.litellm.ai/docs/providers/openai_compatible
        - OpenAI 請求與鑑權約定: https://platform.openai.com/docs/api-reference/making-requests
    ...
    def _parse_market_review_region(cls, value: str) -> str:
        ...
        logging.getLogger(__name__).warning(
            f"MARKET_REVIEW_REGION 配置值 '{value}' 無效，已回退為默認值 'cn'（合法值：cn / hk / us / jp / kr / both；支持逗號分隔有效值）"
        )
        return 'cn'
    ...
    def _parse_market_review_color_scheme(cls, value: str) -> str:
        ...
        if v in ('green_up', 'red_up'):
            return v
        logging.getLogger(__name__).warning(
            "MARKET_REVIEW_COLOR_SCHEME 配置值 '%s' 無效，已回退為默認值 'green_up'（合法值：green_up / red_up）",
            value,
        )
        return 'green_up'
    ...
    def validate_structured(self) -> List[ConfigIssue]:
        ...
        if not self.stock_list:
            issues.append(ConfigIssue(
                severity="error",
                message="未配置 STOCK_LIST。請設置至少一個股票代碼，例如：600519,hk00700,AAPL。",
                field="STOCK_LIST",
            ))
            ...
            if missing_group_stocks:
                issues.append(ConfigIssue(
                    severity="warning",
                    message=(
                        "檢測到 STOCK_GROUP_N 中存在未包含在 STOCK_LIST 內的股票："
                        f"{', '.join(missing_group_stocks[:6])}。"
                        "STOCK_GROUP_N 僅用於郵件路由，不會擴大分析範圍；"
                        "請先將這些股票加入 STOCK_LIST。"
                    ),
                    field="STOCK_GROUP_N",
                ))
        ...
        if not self.tushare_token:
            issues.append(ConfigIssue(
                severity="info",
                message="未配置 Tushare Token，將使用其他數據源",
                field="TUSHARE_TOKEN",
            ))
        ...
        if generation_backend not in SUPPORTED_GENERATION_BACKENDS:
            issues.append(ConfigIssue(
                severity="error",
                message=(
                    "GENERATION_BACKEND 當前支持 "
                    f"{'、'.join(sorted(SUPPORTED_GENERATION_BACKENDS))}。"
                    f"已配置的值為：{generation_backend}。"
                ),
        ...
        if generation_fallback_backend and generation_fallback_backend != LITELLM_BACKEND_ID:
            issues.append(ConfigIssue(
                severity="error",
                message=(
                    "GENERATION_FALLBACK_BACKEND 當前支持 litellm、與 primary 相同的 no-op 值，或空字符串。"
                    f"已配置的值為：{generation_fallback_backend}。"
                ),
        ...
        if agent_generation_backend not in SUPPORTED_AGENT_GENERATION_BACKENDS:
            ...
            issues.append(ConfigIssue(
                severity="error",
                message=(
                    f"AGENT_GENERATION_BACKEND 當前支持 {agent_ui_backends}；"
                    f"local CLI backend（{local_toolless_backends}）僅作為顯式 unsupported diagnostic 保留，"
                    "不支持 Agent 工具調用。"
                    f"已配置的值為：{agent_generation_backend}。"
                ),
        ...
        if local_model_prefix:
            issues.append(ConfigIssue(
                ...
                message=(
                    ...
                    f"請不要使用 LITELLM_MODEL={local_model_prefix}/...。"
                ),
                field="LITELLM_MODEL",
            ))
        if generation_backend == OPENCODE_CLI_BACKEND_ID:
            ...
            if unsafe_model:
                issues.append(ConfigIssue(
                    severity="error",
                    message=(
                        "OPENCODE_CLI_MODEL 是可選的 OpenCode 模型覆蓋值。"
                        "配置時會作為單個 --model 參數傳給 OpenCode，不能包含空白或 shell 元字符；"
                        "不配置時 DSA 將使用 OpenCode 自身默認模型。"
                    ),
        ...
        if not local_generation_backend and not self.llm_model_list and not has_direct_env_model:
            if self.litellm_config_path:
                issues.append(ConfigIssue(
                    ...
                    message=(
                        ...
                        "請檢查 YAML 中的 model_list、litellm_params 和環境變量引用。"
                    ),
                ...
                issues.append(ConfigIssue(
                    severity="error",
                    message=(
                        "未配置任何可用的 AI 模型接入。請至少配置 ANSPIRE_API_KEYS、"
                        "AIHUBMIX_KEY、GEMINI_API_KEY、ANTHROPIC_API_KEY、"
                        "OPENAI_API_KEY 或 DEEPSEEK_API_KEY 中的一個，或配置 "
            ...
            issues.append(ConfigIssue(
                severity="info",
                message=(
                    "尚未明確指定主模型，系統將自動從可用 API Key 推斷。"
                    "建議儘早配置主模型（格式如 gemini/gemini-3.1-pro-preview）"
                ),
                field="LITELLM_MODEL",
            ))
        ...
        if available_router_model_set:
            ...
            if (
                ...
                issues.append(ConfigIssue(
                    severity="error",
                    message=(
                        "已配置的主模型未出現在當前渠道或高級模型路由配置中。"
                        f" 當前可用模型：{', '.join(available_router_models[:6])}"
                    ),
                    field="LITELLM_MODEL",
                ))
            ...
            if configured_agent_primary_model and effective_agent_primary_model:
                ...
                if origins.is_hermes_only:
                    issues.append(ConfigIssue(
                        severity="error",
                        message=(
                            "Hermes-only route 不能作為 Agent 主模型。"
                            "請選擇包含非 Hermes deployment 的 Agent-safe route。"
                        ),
            ...
            if (
                ...
                issues.append(ConfigIssue(
                    severity="error",
                    message=(
                        "已配置的 Agent 主模型未出現在當前渠道或高級模型路由配置中。"
                        f" 當前可用模型：{', '.join(available_router_models[:6])}"
                    ),
                    field="AGENT_LITELLM_MODEL",
                ))
            ...
            if self.vision_model and route_has_hermes(self.llm_model_list, self.vision_model):
                issues.append(ConfigIssue(
                    severity="error",
                    message=(
                        "Hermes Phase 3 未驗證 Vision 能力，VISION_MODEL 不能選擇包含 Hermes deployment 的 route。"
                    ),
                    field="VISION_MODEL",
        ...
        if not self.has_search_capability_enabled():
            issues.append(ConfigIssue(
                severity="info",
                message="未配置搜索引擎能力 (Bocha/MiniMax/Tavily/Brave/SerpAPI/SearXNG)，新聞搜索功能將不可用",
        ...
        if has_telegram_token != has_telegram_chat_id:
            issues.append(ConfigIssue(
                severity="error",
                message="Telegram 通知配置不完整：TELEGRAM_BOT_TOKEN 和 TELEGRAM_CHAT_ID 必須同時配置。",
        ...
        if has_email_sender != has_email_password:
            issues.append(ConfigIssue(
                severity="error",
                message="郵件通知配置不完整：EMAIL_SENDER 和 EMAIL_PASSWORD 必須同時配置。",
        ...
        def _warn_if_webhook_url_invalid(field: str, value: Optional[str]) -> None:
            ...
            issues.append(ConfigIssue(
                severity="warning",
                message=f"{field} 看起來不是有效 URL，請確認是否以 http:// 或 https:// 開頭。",
                field=field,
            ))
        ...
        if self.ntfy_url and not _has_ntfy_topic_endpoint(self.ntfy_url):
            issues.append(ConfigIssue(
                severity="error",
                message="NTFY_URL 必須包含 topic path，例如 https://ntfy.sh/my-topic",
                field="NTFY_URL",
            ))
        ...
        if self.gotify_url and not _has_gotify_base_url(self.gotify_url):
            issues.append(ConfigIssue(
                severity="error",
                message="GOTIFY_URL 必須是 Gotify server base URL，不包含 /message，例如 https://gotify.example",
                field="GOTIFY_URL",
            ))
        ...
        if (
            ...
            issues.append(ConfigIssue(
                severity="warning",
                message="已配置 GOTIFY_URL，但缺少 GOTIFY_TOKEN，Gotify 渠道不會啟用",
                field="GOTIFY_TOKEN",
            ))
        ...
        if self.notification_daily_digest_enabled:
            issues.append(ConfigIssue(
                severity="warning",
                message=(
                    "NOTIFICATION_DAILY_DIGEST_ENABLED 當前為預留配置；"
                    "P4 不會發送每日摘要或持久化摘要內容。"
                ),
        ...
        if (
            ...
            if has_feishu_app_credentials_complete:
                suggestions.append("配置 FEISHU_CHAT_ID 開啟 App Bot 主動推送")
                suggestions.append("開啟 FEISHU_STREAM_ENABLED 使用應用機器人事件訂閱")
            else:
                suggestions.append("補齊 FEISHU_APP_ID / FEISHU_APP_SECRET 後配置 FEISHU_CHAT_ID 開啟 App Bot 主動推送")
            suggestions.append("配置 FEISHU_WEBHOOK_URL 使用自定義機器人 Webhook 推送")
            issues.append(ConfigIssue(
                severity="warning",
                message="僅配置 FEISHU_APP_ID / FEISHU_APP_SECRET 不會開啟飛書靜態通知。"
                        + " 請選擇以下方式之一："
                        + "；".join(suggestions) + "。",
        ...
        if os.getenv("OPENAI_VISION_MODEL"):
            issues.append(ConfigIssue(
                severity="info",
                message=(
                    "OPENAI_VISION_MODEL 已廢棄，請改用 VISION_MODEL。"
                    "當前值已自動遷移，建議更新配置文件以消除此提示。"
                ),
