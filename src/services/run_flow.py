...
_LANES = [
    ...
    {"id": "data_source", "label": "數據來源", "order": 2},
...
def build_task_run_flow_snapshot(
    ...
    *,
    ...
    if flow_status in {"pending", "running", "cancel_requested"}:
        ...
        _append_edge(edges, "task_queue", "analysis_pipeline", "control", flow_status, label="調度")
    else:
...
def build_history_run_flow_snapshot(
    ...
    *,
    ...
    if not any(edge["to"] == "context_pack" for edge in edges):
        _append_edge(edges, "task_queue", "context_pack", "data", context_status, label="輸入")
    ...
    for index, raw_run in enumerate(provider_runs, start=1):
        ...
        if previous:
            ...
            _append_edge(
                ...
                edge_kind,
                status,
                label="降級" if edge_kind == "fallback" else "重試",
                message=_safe_text(run.get("fallback_from") or run.get("fallback_to"), max_length=120),
            )
        else:
            _append_edge(edges, "task_queue", node_id, "control", status, label="調用")
        ...
        _append_event(
            ...
            timestamp=timestamp,
            severity=severity,
            title=f"{label}{'成功' if success else '失敗'}",
            message=message,
            metadata={
                "provider": provider,
                "data_type": data_type,
...
def _append_context_blocks(
    ...
    for block in _as_list(overview.get("blocks")):
        ...
        if provider_node_id:
            _append_edge(edges, provider_node_id, node_id, "data", status, label="輸入")
        else:
            _append_edge(edges, "task_queue", node_id, "data", status, label="輸入")
        _append_edge(edges, node_id, "context_pack", "data", status, label="組裝")
        if status != "success":
            _append_event(
                events,
                ...
                title=f"{block_map.get('label') or key}輸入狀態",
                message=_context_block_message(block_map),
                metadata={
                    "block_key": key,
...
def _append_llm_runs(
    ...
    for index, raw_run in enumerate(llm_runs, start=1):
        ...
        _append_event(
            ...
            title=f"LLM {'成功' if success else '失敗'}",
            message=message,
            metadata={
                "provider": provider,
                "model": model,
...
def _append_history_runs(
    ...
    *,
    ...
    for index, raw_run in enumerate(history_runs, start=1):
        ...
        message = "報告歷史已保存" if success else f"報告歷史保存失敗：{_safe_text(run.get('error_message_sanitized'), max_length=160) or '未知錯誤'}"
        _put_node(
            nodes,
            node_id,
            lane="artifact",
        ...
        _append_event(
            ...
            timestamp=timestamp,
            severity="success" if success else "danger",
            title="歷史保存成功" if success else "歷史保存失敗",
            message=message,
            metadata={
                "metadata_saved": run.get("metadata_saved"),
                "analysis_history_id": run.get("analysis_history_id"),
    ...
    *,
    ...
    for index, raw_run in enumerate(notification_runs, start=1):
        ...
        _append_event(
            ...
            title="通知發送成功" if status == "success" else ("通知跳過" if status == "skipped" else "通知失敗"),
            message=message,
            metadata={
                "channel": channel,
                "status": raw_status,
...
def _put_skeleton_tail(
    ...
    *,
    ...
    _append_edge(edges, anchor_node_id, "context_pack", "data", downstream_status, label="輸入")
...
def _append_task_events(events: List[Dict[str, Any]], task: Any, flow_status: str) -> None:
    _append_event(
        ...
        severity="info",
        title="任務已創建",
        message=getattr(task, "message", None) or "任務已加入隊列",
    )
    if getattr(task, "started_at", None):
        _append_event(
            ...
            severity="info",
            title="任務開始執行",
            message=getattr(task, "message", None) or "任務執行中",
        )
    if flow_status == "failed":
        ...
        _append_event(
            ...
            timestamp=_datetime_to_iso(getattr(task, "completed_at", None)),
            severity="warning",
            title="任務取消" if flow_status == "cancelled" else "任務請求取消",
            ...
            if provider_data_type and provider_run:
                ...
                if previous_provider:
                    ...
                    _append_edge(
                        ...
                        label="降級" if edge_kind == "fallback" else ("重試" if edge_kind == "retry" else "調用"),
                    )
                else:
                    _append_edge(edges, "task_queue", node_id, "control", nodes[node_id].get("status", "unknown"), label="調用")
                last_provider_node_by_type[provider_data_type] = (node_id, provider_run)
            elif event_type in {"llm_run", "llm_run_started"}:
    ...
    events.append(
        {
            ...
            "title": _safe_text(event.get("title"), max_length=100) or "運行事件",
...
def _context_pack_message(overview: Optional[Dict[str, Any]]) -> str:
    if not overview:
        return "未記錄 AnalysisContextPack overview"
    counts = overview.get("counts")
    if isinstance(counts, Mapping):
        available = counts.get("available", 0)
        return f"輸入上下文已組裝，可用塊 {available}"
    return "輸入上下文已組裝"
...
def _llm_run_message(model: Optional[str], run: Dict[str, Any], *, success: bool) -> str:
    ...
    if success:
        if run.get("fallback_model"):
            return f"LLM {display_model} 成功，期間發生模型切換"
        return f"LLM {display_model} 成功"
    error = _safe_text(run.get("error_message_sanitized") or run.get("error_type"), max_length=160)
    return f"LLM {display_model} 失敗：{error or '未知錯誤'}"


def _notification_run_message(channel: str, run: Dict[str, Any], status: str) -> str:
    if status == "success":
        return f"{channel} 通知發送成功"
    if status == "skipped":
        return f"{channel} 通知跳過"
    if status == "failed":
        error = _safe_text(run.get("error_message_sanitized") or run.get("status"), max_length=160)
        return f"{channel} 通知失敗：{error or '未知錯誤'}"
    return f"{channel} 通知結果未知"
