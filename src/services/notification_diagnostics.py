    ...
    if left and not right:
        target.append(
            _issue(
                ...
                f"{channel_name} 已配置 {left_key}，但缺少 {right_key}，該渠道不會啟用。",
                key=right_key,
            )
        )
    if right and not left:
        target.append(
            _issue(
                severity,
                "partial_channel_config",
                f"{channel_name} 已配置 {right_key}，但缺少 {left_key}，該渠道不會啟用。",
                key=left_key,
            )
        )
...
def run_notification_diagnostics(config: Config) -> NotificationDiagnosticResult:
    ...
    info: List[NotificationDiagnosticIssue] = [
        _issue(
            ...
            "釘釘會話和飛書會話屬於運行時消息上下文渠道，無法僅靠靜態 .env 完整判斷。",
        ),
        _issue(
            "info",
            "phase_scope",
            "通知診斷會檢查渠道基線、只讀診斷、Web 測試、P3 路由配置、P4 降噪配置和 P6 ntfy/Gotify 渠道。",
        ),
    ]
    ...
    if not configured:
        errors.append(
            _issue(
                ...
                "0 個通知渠道已配置；如需發送通知，請至少配置一個渠道的 minimal key。",
            )
        )
        ...
        if not ntfy_server_url or not ntfy_topic:
            errors.append(
                _issue(
                    "error",
                    "invalid_ntfy_url",
                    "NTFY_URL 必須包含 topic path，例如 https://ntfy.sh/my-topic。",
                    key="NTFY_URL",
                )
            )
        ...
        if not gotify_endpoint:
            errors.append(
                _issue(
                    "error",
                    "invalid_gotify_url",
                    "GOTIFY_URL 必須是 Gotify server base URL，不包含 /message，例如 https://gotify.example。",
                    key="GOTIFY_URL",
                )
            )
    ...
    if (_has(config, "feishu_webhook_secret") or _has(config, "feishu_webhook_keyword")) and not _has(config, "feishu_webhook_url"):
        warnings.append(
            _issue(
                ...
                "已配置飛書 Webhook 高級安全項，但缺少 FEISHU_WEBHOOK_URL，飛書 Webhook 渠道不會啟用。",
    ...
    if _has(config, "pushplus_topic") and not _has(config, "pushplus_token"):
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置 PUSHPLUS_TOPIC，但缺少 PUSHPLUS_TOKEN，PushPlus 渠道不會啟用。",
                key="PUSHPLUS_TOKEN",
            )
        )
    if _has(config, "ntfy_token") and not _has(config, "ntfy_url"):
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置 NTFY_TOKEN，但缺少 NTFY_URL，ntfy 渠道不會啟用。",
                key="NTFY_URL",
            )
        )
    if (
        ...
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置自定義 Webhook 高級項，但缺少 CUSTOM_WEBHOOK_URLS，自定義 Webhook 渠道不會啟用。",
    ...
    if _has(config, "astrbot_token") and not _has(config, "astrbot_url"):
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置 ASTRBOT_TOKEN，但缺少 ASTRBOT_URL，AstrBot 渠道不會啟用。",
                key="ASTRBOT_URL",
            )
        )
        ...
        if disabled_channels:
            warnings.append(
                _issue(
                    ...
                    (
                        f"{route_config['env_key']} 路由 {route_type} 指向未啟用渠道: "
                        f"{', '.join(disabled_channels)}；這些渠道不會收到該類型通知。"
                    ),
                    key=route_config["env_key"],
                )
            )
        ...
        try:
            ...
            errors.append(
                _issue(
                    "error",
                    "invalid_quiet_hours",
                    f"NOTIFICATION_QUIET_HOURS 配置無效: {exc}",
        ...
        try:
            ...
            errors.append(
                _issue(
                    ...
                    f"NOTIFICATION_TIMEZONE 配置無效: {exc}",
    ...
    if min_severity and not is_supported_notification_severity(min_severity):
        errors.append(
            _issue(
                ...
                (
                    "NOTIFICATION_MIN_SEVERITY 配置無效；"
...
def format_notification_diagnostics(result: NotificationDiagnosticResult) -> str:
    ...
    lines = [
        "通知配置診斷",
        f"已配置渠道: {len(result.configured_channels)} 個",
    ]
