...
def _provider_flow_event(
    ...
    message = (
        ...
        if run.success
        else f"{label} {run.provider} 失敗：{run.error_message_sanitized or run.error_type or '未知錯誤'}"
    )
    return {
        ...
        "type": "provider_run",
        "node_id": node_id,
        "title": f"{label}{'成功' if run.success else '失敗'}",
        "message": sanitize_diagnostic_text(message, max_length=220),
        "metadata": _clean_metadata(
            {
                "trace_id": context.trace_id,
...
def _llm_started_flow_event(
    ...
    *,
    ...
    timestamp = datetime.now().isoformat()
    message = f"LLM {display_model} 調用中"
    return {
        "timestamp": timestamp,
        "severity": "info",
...
def _llm_flow_event(
    ...
    message = (
        f"LLM {model} 成功"
        if run.success
        else f"LLM {model} 失敗：{run.error_message_sanitized or run.error_type or '未知錯誤'}"
    )
    return {
        "timestamp": run.created_at,
        ...
        "type": "llm_run",
        "node_id": node_id,
        "title": f"LLM {'成功' if run.success else '失敗'}",
        "message": sanitize_diagnostic_text(message, max_length=220),
        "metadata": _clean_metadata(
            {
                "trace_id": context.trace_id,
...
def _history_flow_event(
    ...
    message = "報告歷史已保存" if run.report_saved else f"報告歷史保存失敗：{run.error_message_sanitized or '未知錯誤'}"
    return {
        "timestamp": run.created_at,
        "severity": "success" if run.report_saved else "danger",
        "type": "history_run",
        "node_id": node_id,
        "title": "歷史保存成功" if run.report_saved else "歷史保存失敗",
        "message": sanitize_diagnostic_text(message, max_length=220),
        "metadata": _clean_metadata(
            {
                "trace_id": context.trace_id,
...
def _notification_flow_event(
    ...
    if status == "success":
        title = "通知發送成功"
        message = f"{channel} 通知發送成功"
    elif status == "skipped":
        title = "通知跳過"
        message = f"{channel} 通知跳過"
    else:
        title = "通知失敗"
        message = f"{channel} 通知失敗：{run.error_message_sanitized or run.status or '未知錯誤'}"
    return {
        "timestamp": run.created_at,
...
_ANALYSIS_INPUT_STATUS_MESSAGES = {
    "missing": "未進入本次分析輸入",
    "partial": "本次分析輸入僅部分可用",
    "fallback": "本次分析輸入使用降級數據",
    "stale": "本次分析輸入使用過期數據",
    "estimated": "本次分析輸入使用估算數據",
    "fetch_failed": "輸入塊顯示抓取失敗",
    "not_supported": "輸入塊標記為不支持",
}
...
def _analysis_input_status_message(block: Dict[str, Any]) -> Optional[str]:
    ...
    if status == "available" or not status:
        return None
    return _ANALYSIS_INPUT_STATUS_MESSAGES.get(status, f"輸入塊狀態為 {status}")
...
def _provider_component(
    *,
    ...
    if not runs:
        return _component(key, label, "unknown", f"{label}未記錄診斷信息")
    ...
    if successes:
        ...
        if failures:
            return _component(
                ...
                label,
                "degraded",
                f"{label}{provider} 成功，前置數據源失敗後已繼續",
                details,
            )
        return _component(
            key,
...
def _news_component(context_snapshot: Dict[str, Any], raw_result: Dict[str, Any]) -> RunDiagnosticComponent:
    ...
    if isinstance(news_result_count, int):
        if news_result_count > 0:
            if input_message:
                return _component(
                    ...
                    label,
                    "degraded",
                    f"新聞檢索返回 {news_result_count} 條結果，但新聞{input_message}；報告頁相關資訊可能來自後續檢索或歷史持久化",
                    {
        ...
        return _component("news", label, "degraded", "新聞搜索無結果", {"record_count": 0})
    if input_message:
        return _component(
            "news",
            label,
            "unknown",
            f"新聞{input_message}；報告頁相關資訊可能來自後續檢索或歷史持久化",
            {
                "analysis_input_block": "news",
                "analysis_input_status": input_block.get("status"),
                "analysis_input_missing_reasons": _list_text(
    ...
    return _component("news", label, "unknown", "新聞搜索未記錄診斷信息")
...
def _llm_component(diagnostics: Dict[str, Any], raw_result: Dict[str, Any]) -> RunDiagnosticComponent:
    ...
    return _component("llm", label, "unknown", "LLM 未記錄診斷信息")
...
def _history_component(
    ...
    if runs:
        ...
        return _component(
            ...
            label,
            "failed",
            f"報告歷史保存失敗：{last_run.get('error_message_sanitized') or '未知錯誤'}",
        )
    if report_saved is True:
        return _component("history", label, "ok", "報告歷史已保存")
    if report_saved is False:
        return _component("history", label, "failed", "報告歷史保存失敗")
    return _component("history", label, "unknown", "歷史保存未記錄診斷信息")
