...
"""
===================================
交易日曆模塊 (Issue #373 / Issue #1386 P0)
===================================

職責：
1. 按市場（A股/港股/美股/日股/韓股/臺股）判斷當日是否為交易日
2. 按市場時區取“今日”日期，避免服務器 UTC 導致日期錯誤
3. 支持 per-stock 過濾：只分析當日開市市場的股票
4. 提供 regular-session 市場階段推斷基線，不改變現有分析入口行為

依賴：exchange-calendars（可選，交易日判斷不可用時 fail-open，階段推斷不可用時 unknown）
"""
