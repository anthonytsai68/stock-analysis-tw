...
class CustomWebhookSender:
            ...
            try:
                if self._is_discord_webhook(url):
                    ...
                    if fallback_content:
                        ...
                        if self._post_custom_webhook(url, payload, timeout=30):
                            logger.info(
                                "自定義 Webhook %d（圖片不支持，回退文本）推送成功", i + 1
                            )
                            success_count += 1
                    else:
                        logger.warning(
                            "自定義 Webhook %d 不支持圖片，且無回退內容，跳過", i + 1
                        )
            except Exception as e:
        ...
        *,
        ...
        if response.status_code == 200:
            return {
                "channel": "custom",
                "success": True,
                "message": f"自定義 Webhook {index + 1} 測試發送成功",
                "target": url,
                "error_code": None,
                "stage": "notification_send",
                "retryable": False,
    ...
    def _build_custom_webhook_template_payload(self, content: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.error(
                "CUSTOM_WEBHOOK_BODY_TEMPLATE 不是有效 JSON，已回退為默認 Webhook payload: %s",
                exc,
            )
            return None
        if not isinstance(payload, dict):
            logger.error(
                "CUSTOM_WEBHOOK_BODY_TEMPLATE 必須渲染為 JSON object，已回退為默認 Webhook payload"
            )
            return None
        return payload
