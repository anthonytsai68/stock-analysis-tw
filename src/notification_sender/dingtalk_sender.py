...
from src.formatters import chunk_content_by_max_bytes  # <-- 引入項目內置的切片器
...
class DingtalkSender:
    ...
    def send_to_dingtalk(self, content: str, title: str = "", timeout_seconds: int = 10) -> bool:
        """發送 Markdown 消息到釘釘群 (Send DingTalk Markdown message)"""
        if not self.webhook_url:
            return False

        # 1. 簽名邏輯 (Security Signature)
        if self.secret:
        ...
        # 2. 限制標題長度，防止極端長標題吃掉過多 JSON 字節預算
        safe_title = (title[:100] + "...") if title and len(title) > 100 else title

        # 3. 切片邏輯 (Chunking for DingTalk's 20,000 byte limit)
        # 預留 1000 bytes 的安全預算，用於 JSON 結構、標題和分頁後綴的額外開銷
        ...
        for index, chunk in enumerate(chunks):
            ...
            try:
                ...
                if result.get("errcode") == 0:
                    logger.info(f"釘釘消息分段 {index + 1} 發送成功 (Chunk {index + 1} sent successfully)")
                else:
                    logger.error(f"釘釘消息分段 {index + 1} 發送失敗 (DingTalk API error): {result}")
                    all_success = False
            except Exception as e:
                logger.error(f"發送釘釘消息異常 (Failed to send DingTalk notification chunk {index + 1}): {e}")
                all_success = False
