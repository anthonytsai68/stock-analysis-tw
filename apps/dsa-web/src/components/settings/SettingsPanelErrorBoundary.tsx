...
function getSafeErrorSummary(error: unknown) {
  const rawMessage = error instanceof Error
    ...
    : typeof error === 'string'
      ? error
      : '未知前端運行時異常';
  const normalized = rawMessage.replace(/\s+/g, ' ').trim() || '未知前端運行時異常';
...
export const SettingsPanelErrorBoundary = (props: SettingsPanelErrorBoundaryProps) => {
  ...
  const labels: SettingsPanelErrorBoundaryLabels = language === 'en'
    ...
    : {
        loadFailedSuffix: '加載失敗',
        runtimeErrorMessage: '該設置區域發生前端運行時異常，頁面其他設置仍可繼續使用。',
        defaultDiagnosticHint: '請補充 release 版本、運行環境和觸發入口，便於定位問題。',
        errorSummaryPrefix: '錯誤摘要：',
      };
